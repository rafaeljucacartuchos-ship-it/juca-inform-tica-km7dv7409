...
export const getPosVendaMessages = async (filterStr = '', sortStr = '-scheduled_at') => {
  ...
  // Carregamento 100% somente leitura: NENHUMA escrita (PATCH/POST/DELETE) é disparada
  // Todas as criações automáticas agora rodam no servidor via hooks na conclusão da O.S.
...
export async function ensurePostSaleSequenceForCompletedOrders(): Promise<number> {
  // A criação automática das 4 etapas de pós-venda agora é executada 100% no servidor
  // (via pb_hooks no evento de conclusão da O.S., com idempotência e índice único).
  // No-op no cliente para garantir carregamento e navegação estritamente somente-leitura.
  return 0
}
