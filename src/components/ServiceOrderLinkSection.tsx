...
export function ServiceOrderLinkSection({
  ...
  // Fecha dropdown ao clicar/tocar fora: usa pointerdown + mousedown e ignora toques dentro do containerRef
  useEffect(() => {
    ...
    // pointerdown cobre mouse, touch e caneta sem perder o clique em iOS Safari
    if (typeof window !== 'undefined' && 'PointerEvent' in window) {
      document.addEventListener('pointerdown', handlePointerOutside)
      return () => document.removeEventListener('pointerdown', handlePointerOutside)
    }

    // Fallback legado para browsers sem PointerEvent
          ...
          {canEdit && (
            <div className="flex items-center gap-2 self-stretch sm:self-auto">
              <Button
                ...
                size="sm"
                onClick={onUnlinkOs}
                className="w-full sm:w-auto min-h-[44px] sm:min-h-0 h-10 sm:h-8 text-xs sm:text-xs text-rose-600 border-rose-300 hover:bg-rose-50 hover:text-rose-700 font-semibold gap-1.5 touch-manipulation justify-center"
  ...
  // Render do conteúdo dos resultados (compartilhado entre desktop e mobile)
    ...
    >
      ...
      <div className="p-2 sm:p-2 bg-slate-100 text-[11px] font-semibold text-slate-700 flex items-center justify-between border-b border-slate-200">
        ...
        <Button
          ...
          className="min-h-[36px] sm:min-h-0 h-8 sm:h-5 px-2.5 sm:px-1.5 text-xs sm:text-[10px] text-slate-600 hover:text-slate-900 touch-manipulation shrink-0"
        >
          {isInlineMobile ? (
          ...
          results.map((os) => {
            ...
            return (
              <div
                ...
                className="w-full text-left p-3 sm:p-2.5 min-h-[48px] sm:min-h-[44px] hover:bg-indigo-50/80 active:bg-indigo-100 transition-colors flex items-center justify-between gap-3 group cursor-pointer touch-manipulation select-none"
              >
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="text-xs sm:text-[11px] text-slate-700 sm:text-slate-600 font-semibold truncate">
                      {custName}
                    </span>
                  </div>
                ...
                <Button
                  ...
                  className="shrink-0 min-h-[40px] sm:min-h-0 h-9 sm:h-7 px-3 sm:px-2 text-xs sm:text-[10px] font-semibold bg-white border-indigo-300 text-indigo-700 hover:bg-indigo-600 hover:text-white group-hover:bg-indigo-600 group-hover:text-white transition-colors touch-manipulation gap-1"
                >
  ...
  return (
    ...
    >
      ...
      {hasSearchOrFilter && !dropdownOpen && (
        <div className="block sm:hidden pt-1">
          <Button
            ...
            className="w-full min-h-[40px] text-xs text-indigo-700 border-indigo-200 bg-indigo-50/50 hover:bg-indigo-100 flex items-center justify-center gap-1.5 font-semibold touch-manipulation"
          >
      ...
      {/* Mobile: Lista expansível INLINE (sem position absolute para garantir que toque e overflow do Safari nunca falhem) */}
      {dropdownOpen && <div className="block sm:hidden">{renderResultsList(true)}</div>}

      {/* Desktop: Dropdown flutuante absoluto clássico */}
      {dropdownOpen && (
        <div className="hidden sm:flex absolute z-30 left-3 right-3 top-full mt-1 bg-white border border-slate-300 rounded-md shadow-xl overflow-hidden divide-y divide-slate-100 max-h-72 flex-col">
