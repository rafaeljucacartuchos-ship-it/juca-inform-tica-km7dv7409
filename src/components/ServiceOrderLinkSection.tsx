...
export function ServiceOrderLinkSection({
  ...
  const isStatusLocked = status === 'substituido' || status === 'faturado'

  const renderLockedBanner = () => {
    if (!isStatusLocked) return null
    return (
      <div
      ...
      >
        <AlertTriangle
          className={`h-4 w-4 shrink-0 mt-0.5 ${isAdmin ? 'text-amber-600' : 'text-slate-500'}`}
        />
        <div className="leading-snug">
          {isAdmin
            ? 'Orçamento substituído/faturado — como administrador você pode vincular ou ajustar a Ordem de Serviço.'
            : 'Orçamento substituído/faturado — vínculo com Ordem de Serviço em modo somente leitura.'}
        </div>
      </div>
    )
  }
  ...
  return (
    <div ref={containerRef} className="space-y-2 relative">
      {renderLockedBanner()}
      <div className="rounded-lg border border-slate-200 bg-slate-50/70 p-3 sm:p-4 space-y-3 relative shadow-2xs">
