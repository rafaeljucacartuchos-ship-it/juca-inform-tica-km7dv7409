...
export function PropostaPrintDocument({ data, localSignatureUrl }: PropostaPrintDocumentProps) {
  ...
  return (
    <div className="hidden print:block print:w-full print:bg-white print:text-slate-900 print:text-[11px] print:leading-tight">
      <div className="print-document mx-auto max-w-4xl bg-white p-0 text-slate-900">
        ...
        <div className="mb-3.5 rounded-md border-2 border-indigo-200 bg-indigo-50/40 p-3 text-[11px]">
          <div className="mb-1.5 flex items-center justify-between border-b border-indigo-200 pb-1">
            <h3 className="text-xs font-black text-indigo-950 uppercase tracking-wide flex items-center gap-1.5">
              <span>{os?.number ? 'Resumo da Ordem de Serviço' : 'Dados do Atendimento'}</span>
              <span className="text-[10px] font-normal text-indigo-700 lowercase">
                {os?.number ? '(documento integrado)' : '(orçamento independente)'}
              </span>
            </h3>
            <span className="font-mono text-xs font-bold text-indigo-900 bg-white px-2 py-0.5 rounded border border-indigo-200">
              {os?.number ? `${os.number} · ${data.numero_orcamento}` : data.numero_orcamento}
            </span>
          </div>
          <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 leading-snug">
            ...
            <div>
              ...
              <span className="text-slate-900">{equipBrandModel || '—'}</span>
            </div>
            <div>
            ...
            <div>
              ...
              <span className="text-slate-900">
                {os?.attendance_date
                  ...
                  : fmtDate(os?.created || data.created)}
              </span>
            </div>
            {os?.description ? (
              <div className="col-span-2 bg-white/70 p-2 rounded border border-indigo-100">
                ...
                <span className="text-slate-800">{os.description}</span>
              </div>
            ) : null}
          ...
          <table className="w-full border-collapse text-[11px]">
            <thead>
              <tr className="bg-slate-100 text-slate-800 border-b border-slate-300">
                ...
                <th className="px-2 py-1.5 text-left font-bold">Descrição do Produto / Serviço</th>
        ...
        {data.observacoes && (
          <div className="page-break-inside-avoid mb-3.5 rounded-md border border-slate-200 p-2.5 text-[11px]">
            ...
            <p className="whitespace-pre-wrap text-slate-700 leading-relaxed">{data.observacoes}</p>
          </div>
        )}
        ...
        <div className="page-break-inside-avoid mt-6 grid grid-cols-2 gap-8 text-[11px]">
          ...
          <div className="text-center">
            ...
            <p className="mt-1 font-bold text-slate-900">{cust?.name || 'Aprovação do Cliente'}</p>
